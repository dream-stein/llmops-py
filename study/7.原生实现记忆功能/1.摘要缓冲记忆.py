...
class ConversationSummaryBufferMemory:
    ...
    def __init__(self, summary: str = '', chat_histories: list = None, max_tokens: int = 300):
        ...
        self._client = OpenAI(base_url="https://api.longcat.chat/openai/v1")

    @classmethod
    def get_number_tokens(cls, query: str) -> int:
        """计算传入query的token数"""
        return len(query)
    ...
    def save_context(self, human_query: str, ai_content: str) -> None:
        ...
        tokens = self.get_number_tokens(buffer_string)
    ...
    def get_buffer_string(self) -> str:
        ...
        for chat_history in self.chat_histories:
            buffer_string += f"Human:{chat_history.get('human')}\nAI:{chat_history.get('ai')}\n\n"
        return buffer_string.strip()

    def load_memory_variables(self) -> dict[str, Any]:
        """加载记忆变量为一个字典便于格式化到prompt"""
        buffer_string = self.get_buffer_string()
        return {
...
memory = ConversationSummaryBufferMemory("", [], max_tokens=300)
