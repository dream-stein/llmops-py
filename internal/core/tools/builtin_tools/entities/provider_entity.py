...
class ProviderEntity(BaseModel):
    ...
    name: str # 名字
    label: str # 标签，展示给前端看的
    description: str # 描述
    icon: str # 图标
    background: str # 地址
    category: str # 分类
    created_at: int = 0 # 工具创建时间戳
...
class Provider(BaseModel):
    ...
    def _provider_init(self):
        ...
        with open(positions_yaml_path, encoding="utf-8") as f:
            positions_yam_data = yaml.safe_load(f)

        # 3.循环读取位置信息获取服务提供商的工具名字
        for tool_name in positions_yam_data:
            # 获取工具的yaml数据
            ...
            # 5.将工具信息实体幅值填充到tool_entity_map中
