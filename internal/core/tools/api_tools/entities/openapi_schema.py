...
class OpenAPISchema(BaseModel):
    ...
    def validate_paths(cls, paths: dict[str, dict]) -> dict[str, dict]:
        """检验paths数据，涵盖：方法提取、operationId唯一标识、parameters校验"""
        # 1.paths不能为空且类型为字典
        ...
        for interface in interfaces:
            ...
            if not isinstance(interface["operation"].get("description"), str):
                raise ValidateErrorException("description不能为空且字符串")
            ...
            if not isinstance(interface["operation"].get("parameters", []), list):
                raise ValidateErrorException("parameters必须为列表或者为空")
            ...
            for parameter in interface["operation"].get("parameters", []):
                ...
                if (
                    ...
                    raise ValidateErrorException(
                        f"parameter.in必须为{'/'.join([item.value for item in ParameterIn])}"
                    )
                if (
                    not isinstance(parameter.get("in"), str)
                    ...
                    raise ValidateErrorException(
                        f"parameter.type必须为{'/'.join([item.value for item in ParameterType])}"
                    )
            # 9.组装数据并更新
