...
class FunctionCallAgent(BaseAgent):
    ...
    def _llm_node(self, state: AgentState) -> AgentState:
        ...
        try:
            for chunk in llm.stream(state["messages"]):
                ...
                if not generation_type:
                    if chunk.tool_calls:
                        generation_type = "thoughts"
                    elif chunk.content:
                        generation_type = "message"

                # 6.如果生成的是消息则提交智能体消息事件
                if generation_type == "message":
                    # 7.提取片段内容并检测是否开始了输出审核
                    review_config = self.agent_config.review_config
            ...
            print("llm节点发生错误")
            self.agent_queue_manager.publish_error(state["task_id"], f"LLM节点发生错误，错误消息:{str(e)}")
            raise e
        ...
        total_price = (input_token_count * input_price + output_price * output_token_count) * unit
        ...
        if generation_type == "thoughts":
            ...
            # 8.如果LLM直接生成answer则表示已经拿到了最终答案，推送一条空内容用户计算总token+总成本，并停止监听
            self.agent_queue_manager.publish(state["task_id"], AgentThought(
                id=id,
                task_id=state["task_id"],
                event=QueueEvent.AGENT_MESSAGE,
    ...
    def _tools_node(self, state: AgentState) -> AgentState:
        ...
        for tool_call in tool_calls:
            ...
            # 8.判断执行工具的名字，提交不同事件，涵盖智能体动作以及知识库检索
            event = (
    ...
    def _preset_operation_condition(cls, state: AgentState) -> Literal["long_term_memory_recall", "__end__"]:
        """预设操作条件边，用于判断是否出发预设响应"""
        # 1.提取状态的最后一条消息
        message = state["messages"][-1]

        # 2.判断类型的判断，如果是AI消息则说明触发了审核机制，直接结束
        if message.type  == "ai":
            return END
