...
class AgentQueueManager:
    ...
    def listen(self) -> Generator:
        """监听队列返回的生成式数据"""
        # 1.定义基础数据记录超时时间、开始时间、最后一次ping的事件
        listen_timeout = 600
        start_time = time.time()
        last_ping_time = 0

        # 2.创建循环队列执行死循环读取数据，直到超过或者数据读取完毕
        while True:
            try:
                # 3.从队列中提取数据并检测数据是否存在，如果存在则使用yield关键词返回
    ...
    def publish(self, agent_queue_event: AgentQueueEvent) -> None:
        ...
        # 2.检测时间类型是否为需要停止的类型，提供STOP、ERROR、TIMEOUT、AGENT_END
        if agent_queue_event in [QueueEvent.STOP, QueueEvent.ERROR, QueueEvent.TIMEOUT, QueueEvent.AGENT_END]:
            self.stop_listen()
