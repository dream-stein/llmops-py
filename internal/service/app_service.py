...
class AppService(BaseService):
    ...
    def get_draft_app_config(self, app_id: UUID, account: Account) -> dict[str, Any]:
        ...
        # todo: 3.校验model_config配置信息，等待堕LLM实现的时候再来完成

        # 4.循环遍历根据列表删除已经被删除的根据信息
        ...
        for draft_tool in draft_tools:
            if draft_tool["type"] == "builtin_tool":
                ...
                # 8.数据都存在，并且参数以及校验完毕，可以将数据添加到validate_tools
        ...
        # 15.校验知识库列表，如果引入了不存在/被删除的知识库，需要剔除数据并更新，同时获取知识库的额外信息
        datasets = []
        ...
        dataset_dict = {dataset_record.id: dataset_record for dataset_record in dataset_records}
    ...
    def publish_draft_app_config(self, app_id: UUID, account: Account) -> App:
        ...
        max_version = self.db.session.query(func.coalesce(func.max(AppConfigVersion.version), 0)).filter(
            ...
            AppConfigType == AppConfigType.PUBLISHED
        ).scalar()
    ...
    def cancel_publish_app_config(self, app_id: UUID, account: Account) -> App:
        ...
        if app.status != AppStatus.PUBLISHED:
            raise FailException("当且应用未发布，请核实后重试")
    ...
    def get_debug_conversation_summary(self, app_id: UUID, account: Account) -> str:
        ...
        if draft_app_config["long_term_memory"]["enable"] is False:
            raise FailException("该应用未开启长期记忆，无法获取")
    ...
    def delete_debug_conversation(self, app_id: UUID, account: Account) -> App:
        ...
        # 2.判断是否存在debug_conversation_id这个数据，如果不存子啊标识没用会话，无需执行操作
        if not app.debug_conversation_id:
            return app

        # 3.否则将debug_conversation_id值充值为None
    ...
    def debug_chat(self, app_id: UUID, query: str, account: Account) -> Generator:
        ...
        # 5.检测是否开启输入审核，并检测输入，如果存在铭感次则使用预设响应回应并结束代码
        if review_config["enable"] and review_config["review_config"]["enabled"]:
        ...
        # todo:6.根据传递的model_config实体变化不同的LLM模型，等待堕LLM接入后该处会发生变化
        llm = ChatOpenAI(
        ...
        for tool in draft_app_config["tools"]:
            ...
            if tool["type"] == "builtin_tool":
                # 10.内置工具，通过builtin_provider_manager获取工具实例
                ...
                # 11.API工具，首先根据id找到ApiTool记录，然后创建示例
        ...
        # 14.todo:构建Agent智能体，母亲暂时使用functionCallAgent
                ...
                if agent_queue_event.event == QueueEvent.AGENT_MESSAGE:
                    if event_id not in agent_thought:
                        ...
                        agent_thought[event_id] = {
                            ...
                            "answer": agent_queue_event.message,
        ...
        for key, item in agent_thought.items():
            ...
            if item["event"] == QueueEvent.AGENT_MESSAGE:
                ...
                if draft_app_config["long_term_memory"]["enable"]:
                    ...
                    new_conversation_name = debug_conversation
    ...
    def _validate_draft_app_config(self, draft_app_config: dict[str, Any], account: Account) -> dict[str, Any]:
        ...
        # 1.校验上传的草稿配置中对应的字段，至少拥有一个跨域更新的配置
        acceptable_fields = [
        ...
        if "tools" in draft_app_config:
            ...
            for tool in tools:
                ...
                # 6.9 校验对应的工具是否存在，而且需要划分丞builtin_tool和api_tool
                if tool["type"] == "builtin_tool":
        ...
        if "datasets" in draft_app_config:
            ...
            dataset_sets = set([dataset_record.id for dataset_record in dataset_records])
        ...
        if "text_to_speech" in draft_app_config:
            ...
            # 14.2 校验字典类型
            if (
                set(text_to_speech.keys()) != {"enabled", "voice", "auto_play"}
        ...
        if "review_config" in draft_app_config:
            ...
            if (
                ...
                raise ValidateErrorException("review.keywords非空不能超过100个关键词")
            for keyword in review_config["keywords"]:
            ...
            if (
                ...
                raise ValidateErrorException("review.inputs_config格式错误")
            ...
            if (
                ...
                raise ValidateErrorException("review.outputs.config格式错误")
            # 15.7 在开启审核模块的时候，必须确保inputs_config或者outputs_config至少有一个是开启的
            if review_config["enable"]:
                if (
                    review_config["inputs_config"]["enable"] is False
                    and review_config["outputs_config"]["enable"] is False
                ):
                    raise ValidateErrorException("输入审核和输出审核至少续页开启一项")
