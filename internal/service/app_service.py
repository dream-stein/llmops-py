...
class AppService(BaseService):
    ...
    def debug_chat(self, app_id: UUID, query: str, account: Account) -> Generator:
        ...
        # 10.检索是否关联工作流，如果关联工作流则将工作流关联成根据添加到tools中
    ...
    def _validate_draft_app_config(self, draft_app_config: dict[str, Any], account: Account) -> dict[str, Any]:
        ...
        if "workflows" in draft_app_config:
            ...
            ## 7.2 判断管理的工作流列表是否超过5个
            if len(workflows) > 5:
                raise ValidateErrorException("Agent绑定的工作流不能超过5个")
            # 7.3 循环校验工作流的每个参数，列表必须为UUID
            ...
            draft_app_config["workflows"] = [workflow_id for workflow_id in workflows if workflow_id not in workflow_sets]
